...
|---|---|---|
| Mira diagonal vs. hitbox de 4 direções | `directionalHitbox` (`movement.ts`) resolve a mira em 4 quadrantes; a seta visual agora aponta continuamente pro mouse. Num tiro de arco na diagonal, o hitbox pode "errar" um alvo que a seta aponta certeiro. | Junto com o sub-projeto do kit defensivo (passo 3 abaixo), que já vai mexer em `movement.ts`/geometria de hitbox. **Também vira pré-requisito do sub-projeto de animação de ataque** (registrado 07/09/2026): com o hitbox só resolvendo 4 quadrantes mas o sprite/animação de ataque acompanhando 8 direções (mesmo bucketing de `spriteDirection.ts`), um golpe diagonal ficaria visualmente correto mas mecanicamente errado — resolver a geometria do hitbox ANTES de animar ataques, não depois, para não ter que re-tocar timing de animação já ajustado. |
