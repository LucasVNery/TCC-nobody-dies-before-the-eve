...
| `predictThreatMs` modela o tempo de forma contínua; o jogo real avança em passos discretos de 16ms com ordenação "checa distância antes de mover" | Isso introduz um viés de até ~1 passo (16ms) + a distância percorrida nesse passo (alguns ms a poucas dezenas de ms, dependendo da velocidade de perseguição do inimigo), sempre na direção conservadora — o predicado nunca alega mais segurança do que o loop real entrega, só antecipa a ameaça em um valor pequeno. Exemplo medido: Assaltante a 150px, `ASSALTANTE_CHASE_SPEED=90`, `ATTACK_RANGE=60`, `TELEGRAPH_MS=400` → `predictThreatMs` prevê 1400ms, o loop real (`Encounter.step()` em passos de 16ms) só ativa o hitbox em ~1408ms. Coberto por um teste de equivalência em `encounter.test.ts` (spec §2.3 critério 4). | Só importa se algum dia o predicado precisar de precisão sub-frame — a medição da dim 6 de hoje não precisa: a diferença é pequena o bastante pra nunca mudar o veredito `patient`/não-`patient` nos cenários de jogo atuais. |
